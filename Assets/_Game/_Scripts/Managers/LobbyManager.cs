...
namespace Managers
{
    ...
    private void Start() {
        ...
        JoinRoomPanel.JoinPrivateLobby += OnPrivateRoomJoin;
    ...
    private async void OnPrivateRoomJoin(string lobbyName, string lobbyPass)
    {
        using (new Load("Joining Lobby...")) {
            try {
                await MatchmakingService.JoinPrivateLobbyWithAllocation(lobbyName,lobbyPass);
    ...
    private async void OnGameStart() {
        OnSceneLoadServerRpc();
        OnSceneLoadClientRpc();
    ...
    [ServerRpc]
    private void OnSceneLoadServerRpc()
    {
        _roomScreen.gameObject.SetActive(false);
        foreach (var o in disableOnGameLoad)
        {
            o.SetActive(false);
        }
    }
    ...
    [ClientRpc]
    private void OnSceneLoadClientRpc()
    {
        _roomScreen.gameObject.SetActive(false);
        foreach (var o in disableOnGameLoad)
        {
